...
**BL-TPL-007** `form-workflow-selection`（表单工作流选择） 用户需要核对多个语义分组、但可在同一阶段完成时使用分组全页表单；首个业务组从 Tab 下方直接开始，每个业务组必须有独立的无边框白色承载区、组标题和组内字段网格，后续组由灰色工作区间隔。每组四周内边距为 `20px`，标题与首行表单的间距为 `20px`，不显示标题下分隔线。后续工作依赖前一步完成、提交前必须复核，或存在上传-解析-复核-提交的不同状态时使用步骤流程；如果每个填写步骤本身都包含两个及以上需分别核对的业务组，则使用“分阶段分组全页表单”：步骤条常驻在工作区顶部，使用紧凑尺寸，并与下方首个业务组保留 `16px` 工作区留白；当前步骤的业务组与分组全页表单一致，按页面纵向排列，字段标签在上、控件在下，桌面端使用四列网格。采用四列时，每个业务组首行必须由四个有业务含义的字段完整占用，不能留下空列；字段不足四项时应按实际字段数量收缩列数。右侧不得出现引导区。复核步骤的只读汇总也必须置于独立白色业务模块中。 从来源页面进入上述全页表单或步骤流程时，必须在 Shell 新增可关闭 Tab，来源页面留在原 Tab 中。
...
| --- | --- | --- | --- | --- |
| `list.regular` | `list` | 常规查询列表 | 需要按条件查找、比较或处理一组记录，且不需要页面级统计汇总。 | `BL-VIS-012`、`BL-VIS-023`、`BL-VIS-024`、`BL-TPL-001`、`BL-TPL-010`、`BL-TPL-012`、`BL-TPL-013`、`BL-TPL-015`、`BL-TPL-016`、`BL-TPL-022`、`BL-TPL-023`、`BL-INT-001`、`BL-INT-002`、`BL-INT-003`、`BL-INT-004`、`BL-INT-008`、`BL-INT-010`、`BL-INT-018`、`BL-INT-019` |
| `list.inline-summary` | `list` | 行内汇总查询列表 | 查询结果只需 1 至 2 个轻量指标，并且这些指标应服务于结果工具栏扫描。 | `BL-VIS-012`、`BL-VIS-023`、`BL-VIS-024`、`BL-TPL-001`、`BL-TPL-010`、`BL-TPL-011`、`BL-TPL-022`、`BL-TPL-023`、`BL-INT-001`、`BL-INT-003`、`BL-INT-008`、`BL-INT-018`、`BL-INT-019` |
| `list.card-summary` | `list` | 统计卡片查询列表 | 查询结果需要 3 至 5 个重要指标，且统计不承担独立业务操作。 | `BL-VIS-009`、`BL-VIS-012`、`BL-VIS-023`、`BL-VIS-024`、`BL-TPL-001`、`BL-TPL-010`、`BL-TPL-011`、`BL-TPL-022`、`BL-TPL-023`、`BL-INT-001`、`BL-INT-003`、`BL-INT-008`、`BL-INT-018`、`BL-INT-019` |
...
| `form.staged-grouped-flow` | `form` | 分阶段分组全页表单 | 后续工作按步骤推进，且每个填写步骤都包含两个及以上需分别核对的业务信息组；步骤条常驻，当前步骤使用分组全页表单，不显示右侧引导区。 | `BL-TPL-003`、`BL-TPL-005`、`BL-TPL-006`、`BL-TPL-007`、`BL-TPL-008`、`BL-TPL-021`、`BL-VIS-009`、`BL-VIS-019`、`BL-VIS-021`、`BL-VIS-022`、`BL-INT-005`、`BL-INT-006`、`BL-INT-007`、`BL-INT-017` |
...
| 规则包 | 规则引用 |
| --- | --- |
| `core` | `BL-VIS-001`、`BL-VIS-002`、`BL-VIS-004`、`BL-VIS-006`、`BL-VIS-007`、`BL-VIS-008`、`BL-VIS-009`、`BL-VIS-010`、`BL-VIS-015`、`BL-VIS-016`、`BL-VIS-017`、`BL-VIS-018`、`BL-VIS-019`、`BL-VIS-020`、`BL-VIS-021`、`BL-VIS-023`、`BL-INT-011`、`BL-INT-012`、`BL-INT-013`、`BL-INT-014`、`BL-INT-016`、`BL-INT-017`、`BL-INT-019` |
