...
**BL-TPL-016** `column-settings-integrity`（列设置完整性） 每个查询列表的结果 Toolbar 都必须在右上角常驻列设置齿轮；齿轮是右侧操作区的最后一个工具，不随新增、导出、刷新、统计项或字段数量省略。列设置必须使用可用的 Ant Design `Popover` 或 `Dropdown` 与 `Checkbox`，支持业务列显示/隐藏和恢复默认；操作列及列设置本身不能被隐藏。多个同级次要结果操作可合并为一个 `Dropdown`，但新增等唯一主操作必须保持直接可见；删除、失效等危险操作必须独立呈现并按业务影响二次确认，不能藏入 Dropdown。
