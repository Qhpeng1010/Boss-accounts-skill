...
模型先结合业务上下文整理需求并判断是否需要补充；统一入口再根据结构自动判断是否命中已验证配方：查询列表、列表加简单新增、列表加分步骤新增，以及独立分步骤配置都会直接走对应配方。脚本只做等价业务表达的规范化，不根据固定触发词虚构字段或步骤；结构超出配方边界时转入受控自然语言生成，不因未知行操作失败。需要诊断自然语言路径时才显式传入 `--recipe off`；统一入口会在一次调用中返回页面方案、所需规则资源和后续命令，不要再次路由或重新执行入口来获取上下文。
...
```text
业务需求
  -> 模型结合业务上下文整理需求，必要时澄清缺失信息
  -> 统一入口识别页面意图、可用能力并自动判断配方
  -> 命中已验证配方：一次生成规格、预览和静态预检
  -> 未命中配方：读取返回的导演规则、业务规则与精简上下文
  -> 填写 page-spec.json
  -> 固定渲染器生成预览
  -> 需求覆盖、规格契约与静态预检
  -> 人工验收
