...
try {
  ...
  receipt = createTestMcpReceipt(spec.metadata.request, spec.metadata.family);
  ...
  const run = (label, script, args) => {
    const result = spawnSync(process.execPath, [resolve(root, script), ...args], { cwd: root, encoding: 'utf8', stdio: 'inherit', env: testMcpEnvironment(receipt) });
